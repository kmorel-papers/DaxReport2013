...
connetions are given in CGNS order\lcite{CGNS}. An example cell connection
...
could be modfied to be either \sigtag{In} (the default) or
...
control signature argument, \sigtagnum{2} meands data from the second
...
\item[\sigtag{Topology}] This tag corresponds to one of the grid structures
  ...
  \dax{CellTagVoxel}). This is sometimes useful for specializing te  based
...
\begin{daxexample}{Raising an error in the execution environment.}
#include <dax/cont/ArrayHandle.h>
...
\item[\daxmath{Exp}] \index{exponental} Computes $e^x$ where $x$ is the
...
\item[\daxmath{NewtonsMethod}] Uses Newton's method (also known as the
  ...
  \item A \dax{Tuple} to use as the intial guess. If not specified, the
    origin is used.
...
\item[\daxmath{ASinH}] \index{hyperbolic arcsine} \index{inverse hyperbolic sine}
  ...
  component-wise operation.
\item[\daxmath{ATan}] \index{arctangent} \index{inverse tanget} Returns the
...
\item[\daxmath{Tan}] \index{tangent} Returns the tanget of an angle given
...
\begin{longtable}{lc}
  ...
  & \fbox{\begin{minipage}[c][2in]{2in}\fix{Hexahedron}\end{minipage}} \\
  \dax{CellTagLine} \index{line}
  & \fbox{\begin{minipage}[c][2in]{2in}\fix{Line}\end{minipage}} \\
  \dax{CellTagQuadrilateral} \index{quadrilateral}
  & \fbox{\begin{minipage}[c][2in]{2in}\fix{Quadrilateral}\end{minipage}} \\
  \dax{CellTagTetrahedron} \index{tetrahedron}
  & \fbox{\begin{minipage}[c][2in]{2in}\fix{Tetrahedron}\end{minipage}} \\
  \dax{CellTagTriangle} \index{triangle}
  & \fbox{\begin{minipage}[c][2in]{2in}\fix{Triangle}\end{minipage}} \\
  \dax{CellTagVertex} \index{vertex}
  & \fbox{\begin{minipage}[c][2in]{2in}\fix{Vertex}\end{minipage}} \\
  \dax{CellTagVoxel} \index{voxel}
  & \fbox{\begin{minipage}[c][2in]{2in}\fix{Voxel}\end{minipage}} \\
  \dax{CellTagWedge} \index{wedge}
  & \fbox{\begin{minipage}[c][2in]{2in}\fix{Wedge}\end{minipage}} \\
\end{longtable}
...
tags and opearte on \textidentifier{CellField} containers.
...
and a \dax{Vector3} contining parametric coordinates. It returns the
